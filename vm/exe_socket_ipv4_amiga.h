...
*
* Purpose:  socket functions IP V4 for Amiga OS
*
...
#if OS_AROS
    #define LOCK();              LockMutex (socket_mutex);
    #define UNLOCK();            UnlockMutex (socket_mutex);
#else
    #define LOCK();              pthread_mutex_lock (&socket_mutex);
    #define UNLOCK();            pthread_mutex_unlock (&socket_mutex);
#endif
...
U1 exe_open_accept_server (S8 slist_ind, S8 *new_slist_ind)
{
    ...
    socklen_t len = sizeof (client);        /* Needed hours to find this out!!! AAAAAAHRG!! Fixed the EBADF bug now. */
    S4 i, socketh = -1;
    ...
    connection = accept (sock[slist_ind].sock, (struct sockaddr *) &client, &len);
    if (connection == -1)
    {
        return (errno);
    }
    ...
    *new_slist_ind = socketh;
    ...
    strcpy (sock[*new_slist_ind].client_ip, inet_ntoa (client.sin_addr));
...
U1 exe_open_client (S8 *slist_ind, U1 *hostname, S8 port, S4 threadnum)
{
    ...
    *slist_ind = socketh;
    ...
    client = socket (PF_INET, SOCK_STREAM, 0);  /* PF_INET */
    if (client == -1)
    {
        UNLOCK();
        return (errno);
...
U1 exe_close_server (S8 slist_ind)
{
    ...
    if (shutdown (sock[slist_ind].sock, 0x02) == -1)
    {
        return (ERR_FILE_CLOSE);
    }
...
U1 exe_close_accept_server (S8 slist_ind)
{
    ...
    if (shutdown (sock[slist_ind].serv_conn, 0x02) == -1)
    {
        return (ERR_FILE_CLOSE);
    }
